...
export function SettingsClient({ initialData, initialTab = 'perfil', paymentRequired = false }: SettingsClientProps) {
  ...
  const handleUpgrade = async (plan: string) => {
    ...
    try {
      const { createCheckoutAction } = await import('../_actions/create-checkout')
            ...
            <form onSubmit={handleSubmit(onSubmit)}>
              ...
              {activeTab === 'pagamento' && (
                <div className="space-y-8">
                    ...
                    {[
                      {
                        ...
                        features: ['5 orçamentos/mês', 'Catálogo de 20 itens', 'PDF profissional'],
                      ...
                      },
                      {
                        ...
                        features: ['Ilimitado', 'Link dinâmico', 'Suporte prioritário'],
                      ...
                      },
                      {
                        ...
                        features: ['Tudo do Pro', 'Até 5 membros', 'Painel Admin'],
                        color: 'bg-gray-900 text-white',
                      },
                    ].map((plan) => {
                      const isCurrent = (initialData.organization.subscription?.plan || initialData.organization.metadata.plan || 'free') === plan.id
                      return (
                        <Card key={plan.id} className={cn(
                          "relative p-6 border transition-all flex flex-col",
                          isCurrent ? "border-brand shadow-lg ring-1 ring-brand/10" : "border-gray-100 hover:border-gray-200"
                        )}>
                          {plan.popular && (
                          ...
                          <div className="mb-6">
                            <div className={cn("inline-flex px-2.5 py-1 rounded-lg text-[10px] font-bold uppercase mb-4", plan.color)}>
                              {plan.name}
                            </div>
                          ...
                          <ul className="space-y-2 mb-8 flex-1">
                            {plan.features.map(f => (
                              <li key={f} className="flex items-center gap-2 text-[11px] text-gray-600">
                                <CheckCircle2 className="w-3.5 h-3.5 text-brand" /> {f}
                              </li>
                            ))}
                          </ul>
                          ...
                          {isCurrent ? (
                            ...
                            <Button
                              ...
                              className={cn(
                                ...
                                plan.id === 'equipe' ? "bg-gray-900 hover:bg-black text-white" : "bg-brand hover:bg-brand-dark text-white"
                              )}
                            >
                  ...
                  <div className="pt-6 border-t border-gray-100">
                    <h3 className="text-sm font-bold text-gray-900 mb-4 uppercase tracking-widest">Histórico de Faturas</h3>
                    <div className="overflow-hidden border border-gray-100 rounded-xl">
                      <table className="w-full text-left text-sm">
                        <thead className="bg-gray-50/50 border-b border-gray-100">
                          <tr>
                            <th className="px-6 py-4 font-semibold text-gray-900">Data</th>
                            <th className="px-6 py-4 font-semibold text-gray-900">Valor</th>
                            <th className="px-6 py-4 font-semibold text-gray-900">Status</th>
                            <th className="px-6 py-4 font-semibold text-gray-900 text-right">Ação</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-50">
                          {initialData.organization.billingHistory.length > 0 ? (
                            initialData.organization.billingHistory.map(invoice => (
                              <tr key={invoice.id}>
                                ...
                                <td className="px-6 py-4 text-right">
                                  <Button variant="ghost" size="sm" className="text-brand text-xs font-bold">PDF</Button>
                                </td>
                              </tr>
                            ))
                          ) : (
