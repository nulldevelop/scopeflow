          ...
          <Card className="p-8 bg-white border border-gray-200 rounded-[20px]">
            {activeTab === 'perfil' && (
              <div className="space-y-8">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900 mb-1">Informações do Perfil</h3>
                  <p className="text-sm text-gray-500">Atualize seus dados pessoais e profissionais.</p>
                </div>
                ...
                <div className="flex items-center gap-6">
                  <div className="w-20 h-20 rounded-2xl bg-brand-light flex items-center justify-center overflow-hidden border-2 border-white shadow-sm">
                    {user.avatar ? (
                      ...
                      <span className="text-brand font-bold text-2xl">{user.nome.charAt(0)}</span>
                    )}
                  </div>
                  <div>
                    <Button variant="outline" size="sm" className="mb-2">Alterar foto</Button>
                    <p className="text-[11px] text-gray-400">JPG, GIF ou PNG. Máximo de 2MB.</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-2">
                    ...
                    <Input id="nome" defaultValue={user.nome} className="rounded-lg h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="email">E-mail</Label>
                    <Input id="email" defaultValue={user.email} className="rounded-lg h-11" />
                  </div>
                  <div className="space-y-2">
                    <Label htmlFor="valorHora">Valor da Hora (R$)</Label>
                    <Input id="valorHora" type="number" defaultValue={user.valorHora} className="rounded-lg h-11" />
                  </div>
                  <div className="space-y-2">
            ...
            {activeTab !== 'perfil' && (
              <div className="py-12 text-center">
                ...
                <p className="text-sm text-gray-500 max-w-sm mx-auto">
                  Esta seção de configurações está sendo preparada e estará disponível em breve.
                </p>
              </div>
            )}
          </Card>
